...
{
    const int required_num_of_devices = 2;
    const int min_compute_capability  = 7;
    GMX_MPI_TEST(RequireRankCount<required_num_of_devices>);
    int      rank;
    ...
    if (testDeviceList.size() < required_num_of_devices)
    {
        GTEST_SKIP() << "Requested " << required_num_of_devices << " device(s), "
    ...
    if (deviceInfo.prop.major < min_compute_capability)
    {
        GTEST_SKIP() << "NVSHMEM requires minimum Volta or higher GPUs";
    }
