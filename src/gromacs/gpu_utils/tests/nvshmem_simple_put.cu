 ...
 *
 * \param[out] out_destination Buffer to write the output into.
 * \param[in]  size            Size of the data buffers.
 *
 */
static __global__ void nvshmem_simple_put(DeviceBuffer<int> out_destination, const int size)
{
...
#    if __CUDA_ARCH__ >= 700
    ...
    if (threadIdx.x < size)
    {
        nvshmem_int_p(out_destination + threadIdx.x, mype, peer);
    }
#    endif
}
...
void nvshmemRunSimplePut(const TestDevice* testDevice)
{
    ...
    auto       kernelPtr = nvshmem_simple_put;
    const auto kernelArgs =
            prepareGpuKernelArguments(kernelPtr, kernelLaunchConfig, &destination, &numElements);
    launchGpuKernel(kernelPtr, kernelLaunchConfig, deviceStream, nullptr, "nvshmem_simple_put", kernelArgs);
    ...
    freeDeviceBufferNvShmem(&destination);
    nvshmem_finalize();
