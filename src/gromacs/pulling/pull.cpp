    ...
    for (gmx::Index i = gmx::ssize(pull->coord) - 1; i >= 0; i--)
    {
        ...
        if (pcrd.params.eGeom == PullGroupGeometry::Transformation)
        {
            gmx::distributeTransformationPullCoordForce(
                    &pcrd, gmx::ArrayRef<pull_coord_work_t>(pull->coord).subArray(0, pcrd.params.coordIndex));
        }
...
void dd_make_local_pull_groups(const t_commrec* cr, struct pull_t* pull)
{
    ...
    for (pull_group_work_t& group : pull->group)
    {
        ...
        {
            group.localWeightFactors.resize(group.atomSet.numAtomsLocal());
            for (size_t i = 0; i < group.atomSet.numAtomsLocal(); ++i)
            {
                group.localWeightFactors[i] =
                        group.globalWeightFactors[group.atomSet.collectiveIndex()[i]];
            }
        }
                                  ...
                                  real               lambda)
{
    ...
    const bool setWeightFactors = !pg->params.weightFactorExpression.empty();
