...
void gmx::LegacySimulator::do_md()
{
    ...
    else
    {
        ...
        mdAlgorithmsSetupAtomData(cr_, *ir, topGlobal_, top_, fr_, &f, mdAtoms_, constr_, vSite_, shellfc);
    ...
    wallcycle_start(wCycle_, WallCycleCounter::Run);
    ...
    while (!bLastStep)
    {
        ...
        wallcycle_start(wCycle_, WallCycleCounter::Step);
        ...
        if (bNS && simulationWork.havePpDomainDecomposition && simulationWork.useGpuHaloExchange)
        {
            ...
            constructGpuHaloExchange(*cr_, *fr_->deviceStreamManager, wCycle_);
        }
        ...
        if (bExchanged)
        {
            ...
            cglo_flags = CGLO_GSTAT | CGLO_TEMPERATURE;
            compute_globals(gstat,
                            cr_,
                            ir,
                            fr_,
            ...
            || !mdGraph->useGraphThisStep())
        {
            ...
            if (EI_VV(ir->eI))
            {
                integrateVVFirstStep(step,
                                     ...
                                     fpLog_,
                                     wCycle_);
                if (vSite_ != nullptr && needVirtualVelocitiesThisStep)
                {
                    // Positions were calculated earlier
                    wallcycle_start(wCycle_, WallCycleCounter::VsiteConstr);
                    vSite_->construct(state_->x, state_->v, state_->box, VSiteOperation::Velocities);
                    wallcycle_stop(wCycle_, WallCycleCounter::VsiteConstr);
                }
            }
            ...
            if (runScheduleWork_->stepWork.useGpuFBufferOps
                && (simulationWork.useGpuUpdate && !vSite_) && do_per_step(step, ir->nstfout))
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    upd.update_sd_second_half(*ir,
                                              ...
                                              do_ene);
                    upd.finish_update(
                            *ir, md->havePartiallyFrozenAtoms, md->homenr, state_, wCycle_, constr_ != nullptr);
                }
        ...
        cycles = wallcycle_stop(wCycle_, WallCycleCounter::Step);
        if (haveDDAtomOrdering(*cr_) && wCycle_)
        {
        ...
        resetHandler->resetCounters(
                step, step_rel, mdLog_, fpLog_, cr_, fr_->nbv.get(), nrnb_, fr_->pmedata, pme_loadbal, wCycle_, wallTimeAccounting_);
