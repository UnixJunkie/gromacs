...
void gmx::LegacySimulator::do_mimic()
{
    ...
    while (!isLastStep)
    {
        ...
        if (MAIN(cr_))
        {
            const bool constructVsites = ((vSite_ != nullptr) && mdrunOptions_.rerunConstructVsites);
            ...
            if (constructVsites)
            {
                ...
                vSite_->construct(state_->x, state_->v, state_->box, VSiteOperation::PositionsAndVelocities);
